...
def get_or_head(reqDict, method):
    ...
    # get headers, uri from the reqDict
    headers = reqDict.get("headers")
    uri = reqDict.get("uri")
    # urlparse components -> path,scheme,netloc,query,fragment
    ...
    else: # file exists at path
        # if client not have permission
    ...
    # generate ETag
    ETag = '"' + \
        hashlib.md5((str(os.path.getmtime(path)).encode())).hexdigest() + '"' # get last modified time
    ...
    elif "If-Unmodified-Since" in headers.keys(): # if not modified earlier
    ...
    if flag and "Range" in headers.keys() and (headers["Range"].split("="))[0] == "bytes": # byte-range request
        dataAvailable = True
        ...
        if "If-Range" in headers.keys():
            ...
            # If-Range header is in last-modified format
                ...
                timeFromHeader = time.mktime(date.timetuple())
                lastModifiedTime = os.path.getmtime(path) - TIME_DIFF
                # full resource should be returned instead of partial content
                if timeFromHeader < lastModifiedTime:
                    dataAvailable = False
        ...
        if dataAvailable:
            ...
            if len(rangesList) > 1:
                ...
                if int(startPos) <= int(endPos):
                    ...
                    if (method == "GET"):
                        responseDict["body"] = body
                        responseDict["headers"]["Content-MD5"] = hashlib.md5(
                            responseDict["body"]).hexdigest()
                    return responseDict
    responseDict["isError"] = False
    ...
    responseDict["headers"]["Expires"] = utility.toRFC_Date(
        datetime.fromtimestamp(int(time.time()) + EXPIRE_TIME))
...
def post(reqDict):
    ...
    contentEncoding.split(",").reverse() # to process each encoding in correct order
    ...
    if doesfileExist:
        # generate ETag
        ETag = '"' + \
            hashlib.md5((str(os.path.getmtime(path)).encode())).hexdigest() + '"'
        if "If-Match" in headers.keys():
            if ETag not in ifMatchArr and ifMatchArr[0] != "*":
                return {"isError": True, "Status-Code": 412, "Status-Phrase": "Precondition Failed", "Msg": "Could not match given ETag."}
    ...
    if "application/x-www-form-urlencoded" in contentType:
        ...
        if int(statusCode) == 303:
            responseDict["headers"]["Location"] = "http://localhost:" + \
                str(SERVER_PORT) + "/postSuccess.html"
        responseDict["headers"]["Content-Length"] = 0
        return responseDict
...
def put(reqDict):
    ...
    if doesFileExist:
        ...
        if "If-Match" in headers.keys():
            ...
            date = datetime.strptime(
                headers["If-Unmodified-Since"], "%a, %d %b %Y %H:%M:%S GMT")
            timeFromHeader = time.mktime(date.timetuple())
            lastModifiedTime = os.path.getmtime(path) - TIME_DIFF
            if timeFromHeader < lastModifiedTime:
                return {"isError": True, "Status-Code": 412, "Status-Phrase": "Precondition Failed", "Msg": "Could not meet If-Unmodified-Since header requirements."}
    ...
    if "application/x-www-form-urlencoded" in contentType:
        ...
        responseDict["isError"] = False
        responseDict["headers"] = {}
        responseDict["headers"]["Content-Location"] = path.split(DEFAULT_DIR_PATH)[
            1]
        responseDict["headers"]["Expires"] = utility.toRFC_Date(
            datetime.fromtimestamp(int(time.time()) + EXPIRE_TIME))
        responseDict["headers"]["Content-Length"] = 0
...
def delete(reqDict):
    ...
    if os.path.exists(path):
        if os.path.isfile(path):
            ...
            statusPhrase = "Accepted"
            if not os.access(path.rsplit("/", 1)[0], os.W_OK):
                return {"isError": True, "Status-Code": 403, "Status-Phrase": "Forbidden", "Msg": "Client donot have the permission to delete this folder."}
            else:
